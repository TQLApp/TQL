...
{
    Write-Host "Compressing app"
    
    Start-Process `
        -NoNewWindow `
        -Wait `
        -FilePath ($Global:Root + "\Build\Libraries\7z\7za.exe") `
        -ArgumentList ("a -bd -bso0 -bse0 Archive.7z `"" + $Global:Distrib + "\Bin\*`"") `
...
Function Create-Setup()
{
    ...
    $Config = @"
;!@Install@!UTF-8!
Title="Techie's Quick Launcher Setup"
RunProgram="Tql.Setup.App.exe"
Progress="no"
;!@InstallEnd@!
"@

    $Target = $Global:Distrib + "\Setup.exe"
    $Header = $Global:Root + "\Build\Support\Header.sfx"
    $Payload = $Global:Distrib + "\Archive.7z"
...
Copy-Output -From "Tql.Setup.App" -Target ($Global:Distrib + "\Bin")
Copy-Output -From "Tql.App" -Target ($Global:Distrib + "\Bin\Assets")
