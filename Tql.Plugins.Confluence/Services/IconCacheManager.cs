...
internal class IconCacheManager
{
    ...
    {
        _iconCache = Path.Combine(store.GetCacheFolder(ConfluencePlugin.Id), "Icons");
    ...
    public async Task<ImageSource?> LoadIcon(ConfluenceClient client, string url)
    {
        if (string.IsNullOrEmpty(url))
            return null;
    ...
    private Task<ImageSource> DownloadImage(ConfluenceClient client, string url)
    {
